...
func NewFieldSelector(dest any) (*fieldSelector, error) {
	...
	fs.parser, err = sql.NewSQLParser(
		...
		sql.WithOverrideFunction("K", fs.queryKey),
	)
	if err != nil {
...
func (fs *fieldSelector) ParseFromString(ctx context.Context, input string) (string, []string, error) {
...
func (fs *fieldSelector) Parse(ctx context.Context, selector fields.Selector) (string, []string, error) {
	return fs.parser.Parse(ctx, selector)
}
...
type resolverFunc func(string) *queryparser.TokenSet
...
func (fs *fieldSelector) resolveQuery(operator selection.Operator, schemaField *schema.Field, resolve resolverFunc) (*queryparser.TokenSet, error) {
...
func (fs *fieldSelector) resolveField(field SelectorFieldName, schemaField *schema.Field, resolve resolverFunc) (*queryparser.TokenSet, error) {
	switch schemaField.DataType {
	case "jsonb":
...
func (fs *fieldSelector) resolveValue(schemaField *schema.Field, value string, resolve resolverFunc) (*queryparser.TokenSet, error) {
	switch schemaField.DataType {
	case schema.Time:
		if _, err := time.Parse(time.RFC3339, value); err != nil {
			return nil, err
		}
		return queryparser.NewTokenSet().AddFunctionToken("CAST", func(ts *queryparser.TokenSet) {
			ts.Append(resolve(value))
			ts.AddLiteralToken("timestamp")
		}), nil
	default:
...
func (fs *fieldSelector) applyTextArrayOperator(operator selection.Operator, resolve resolverFunc) (*queryparser.TokenSet, error) {
	switch operator {
...
func (fs *fieldSelector) applyTimestampOperator(operator selection.Operator, resolve resolverFunc) (*queryparser.TokenSet, error) {
	switch operator {
