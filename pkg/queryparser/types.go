...
type Parser interface {
	...
	//   - []string: Additional data extracted during parsing.
	//   - error: An error object providing details if the parsing fails.
	Parse(ctx context.Context, input any, params ...string) (string, []string, error)
}
...
// AddLiteralToken adds a literal token with the specified value to the TokenSet.
func (s *TokenSet) AddLiteralToken(value string) {
	*s = append(*s, Token{
		Type:  TokenLiteral,
		Value: value,
	})
...
// QueryArgLiteral represents a literal argument in a query.
type QueryArgLiteral struct {
	val string
}

// Value returns the string value of the QueryArgLiteral.
func (qa *QueryArgLiteral) Value() string {
	return qa.val
}
...
// Each argument can be a literal value or another function.
