...
func Parse(ctx context.Context, input any, params []string, options ...ParserOption) (*QueryFunc, error) {
	var err error
	...
	for i, token := range tokens {
		...
		switch token.Type {
		case TokenFunc:
			opStack = append(opStack, &QueryFunc{name: token.Value})
		...
		case TokenLiteral:
			...
			if isParam(v) {
				...
				if err := validateParamNumber(n, len(params), i); err != nil {
					return nil, fmt.Errorf("param number %d is out of bounds at token position %d", i, n)
				}
				v = params[n-1]
			}
			opStack[len(opStack)-1].args = append(opStack[len(opStack)-1].args, &QueryArgLiteral{v})
...
// Tokenize breaks the input into tokens. It recognizes function names and literals.
//
// Example Input:
// "AND(EQ(key1, val1), OR(NEQ(key2, val2), NEQ(key3, val3)))"
//
// This input contains:
//
// - Function names: AND, EQ, OR, NEQ
//
...
func Tokenize(ctx context.Context, input string) (TokenSet, error) {
	...
	for i, char := range input {
		...
		case ')':
			if currentStr := strings.TrimSpace(current.String()); currentStr != "" {
				tokens = append(tokens, Token{Type: TokenLiteral, Value: currentStr})
				current.Reset()
			}
			if openParenCount == 0 {
		...
		case ',':
			if currentStr := strings.TrimSpace(current.String()); currentStr != "" {
				if openParenCount == 0 {
					return nil, fmt.Errorf("literal without a function at position %d", pos)
				}
				tokens = append(tokens, Token{Type: TokenLiteral, Value: currentStr})
				current.Reset()
			} else if !expectComma {
			...
			expectComma = false // After a comma, expect a new function or literal
		default:
...
// IsLiteral checks if the provided QueryArg is of type QueryArgLiteral.
func IsLiteral(arg QueryArg) bool {
	switch arg.(type) {
	case *QueryArgLiteral:
		return true
	default:
		return false
	}
...
func verifyTokens(tokens TokenSet, funcs QueryFuncSet) error {
	...
	for i, token := range tokens {
		...
		case TokenFunc:
			...
			currentFunc, exists := funcs[token.Value]
			if !exists {
			...
			if currentFunc.UsedBy.Size() > 0 {
				parentFunc := RootFunc
				if len(opStack) > 0 {
					parentFunc = opStack[len(opStack)-1].Value
				}
