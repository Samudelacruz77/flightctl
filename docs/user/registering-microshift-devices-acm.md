...
Flight Control includes a configuration provider that can retrieve data from an HTTP(S) endpoint. This integrates with ACM's agent registration API, requiring it to be formatted as a Flight Control repository.
...
Note that this repository will show as not valid, because the root URL does not return a value directly. However, this endpoint will be used in the fleet spec configuration section with a set of suffixes that will return the required Kubernetes manifests for auto-registration.
...
The fleet specification is composed of three main sections: OS, config and hooks.

1. **Operating System (OS) Image**: This section defines the target Operating System image required for all devices, in this case an image containing the MicroShift runtime. This is essential for running the Klusterlet, the agent responsible for communication with ACM.
...
1. **Configuration**: The configuration section uses the HTTP configuration provider to fetch information from an endpoint. The repository `acm-test` contains the registration URL for ACM's agent registration:
    ...
    This retrieves the CRD from the ACM endpoint and stores it at the specified file path.

    Basically, it is like making an API call against the endpoint:

    `https://$agent_registration_host/agent-registration/crds/v1`
